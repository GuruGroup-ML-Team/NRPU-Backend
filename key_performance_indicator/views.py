...
def key_performance_indicator(request):
    try:
        ...
        if indicator == "I. Key Performance Indicators" and sub_indicator == 'All':
            if sector == 'All':
                ...
                df = df.drop_duplicates(subset=['Indicator','Sub Indicator'], keep='first').reset_index(drop=True)
            else:
                # Filter for the specified indicators within the specific sector
                df = df[(df['Sector'] == sector) & (df['Indicator'] == indicator)]
                # If a specific sector and 'All' indicator, return first occurrence per indicator for that sector
                df = df[df['Sector'] == sector].drop_duplicates(subset=['Sub Indicator'], keep='first').reset_index(drop=True)
            df = df[['Sector', 'Sub-Sector', 'Org Name', 'Indicator', 'Sub Indicator', 'Sub-Sub Indicator'] + selected_years]
        
        elif indicator == "I. Key Performance Indicators" and sector == 'All':
            ...
            if indicator == "I. Key Performance Indicators":
            ...
            df = df.drop_duplicates(subset=['Sub Indicator'], keep='first')
