...
urlpatterns = [
    ...
    path('organization/fetch-specific-org/', FetchSpecificOrgDataView.as_view(), name='fetch_specific_org_data'),
    path('organization/fetch-by-sector/', FetchSectorDataView.as_view(), name='fetch_by_sector_data'),

    # Endpoints for Financial Variables (Logic Two)
    path('financial-variables/org-specific/', OrgSpecificFinancialVariablesView.as_view(), name='org_specific_financial_variables'),
    path('financial-variables/sector-average/', SectorAverageFinancialVariablesView.as_view(), name='sector_average_financial_variables'),

    # Endpoints for Financial Ratios (Logic Three)
    path('financial-ratios/company-ratios/', CompanyRatiosView.as_view(), name='company_ratios'),
    path('financial-ratios/sector-ratios/', SectorRatiosView.as_view(), name='sector_ratios'),

    # Endpoints for Ratio Comparison (Logic Four)
    path('ratio-comparison/compare-ratios/', RatioComparisonView.as_view(), name='compare_ratios'),

    # Endpoints for Overall Score (Logic Five)
    path('overall-score/calculate/', OverallScoreView.as_view(), name='calculate_overall_score'),
]
