...
import pandas as pd
# Assuming RatioComparisonService is correctly imported and available
from .RatioComparisonService import ratio_comparison_service_instance
...
class OverallScoreService:
    ...
    # --- Define your hardcoded latest year here ---
    # IMPORTANT: REMEMBER TO UPDATE THIS VALUE ANNUALLY OR AS NEW DATA BECOMES AVAILABLE
    DEFAULT_LATEST_YEAR = "2023" 
    ...
    def calculate_overall_score(self, 
                                 ...
                                 sub_sector: str, 
                                 org_name: str, 
                                 weight_type: str, # Required parameter, moved before optional ones
                                 year: int | str | None = None, # Optional parameter, default to None
                                 user_type: str = None,         # Optional parameter
                                 custom_weights: dict = None    # Optional parameter
                                 ) -> tuple[dict | None, dict, dict, str | dict]:
        """
        Calculates the overall weighted financial score for a company or bank.
        Returns a dictionary containing the raw score and its percentage equivalent if a specific year is requested,
        or a dictionary of {year: {raw_score, percentage_score}} if 'all' years are requested.
        ...
        Args:
            ...
            sub_sector (str): The sub-sector of the organization.
            org_name (str): The name of the organization.
            weight_type (str): 'default' or 'custom'. (Required)
            year (int | str | None): The year for which to perform the comparison. Can be an integer, 'all', or None (for default latest year).
            user_type (str, optional): "Lender" or "Investor". Required if weight_type is 'default'.
            custom_weights (dict, optional): A dictionary of custom weights. Required if weight_type is 'custom'.
        ...
        # If year is None, use the hardcoded default
        actual_year = None
        if year is None:
            actual_year = self.DEFAULT_LATEST_YEAR
            print(f"No year specified in request, using hardcoded default year: {actual_year} for {org_name}.")
        else:
            # Ensure the year is a string, as 'all' or integer strings are expected by RatioComparisonService
            actual_year = str(year)
        
        is_all_years = (actual_year.lower() == 'all')

        # This call determines the structure of category_scores_data
        category_scores_data = ratio_comparison_service_instance.compare_financial_ratios(
            entity_type=entity_type,
            sector=sector,
            sub_sector=sub_sector,
            org_name=org_name,
            year=actual_year # Pass the determined actual_year here
        )
        ...
        if not category_scores_data:
            print(f"Failed to retrieve category scores from ratio comparison for year {actual_year}. Cannot calculate overall score.")
            return None, {}, detailed_breakdown, overall_score_interpretation
        ...
        if weight_type.lower() == 'default':
            ...
            total_custom_weight = sum(custom_weights.values())
            if not (0.99 <= total_custom_weight <= 1.01): # Allow for floating point inaccuracies
                print(f"Error: Custom weights do not sum to 1.0 (Sum: {total_custom_weight:.2f}).")
                return None, {}, detailed_breakdown, overall_score_interpretation
            
            # Use "Lender" as a fallback user_type for default_categories_ref if user_type is not provided for custom weights
            # This is just to get the valid category names for validation.
            default_categories_ref = self._get_default_weights(entity_type, user_type if user_type else "Lender").keys() 
            for category in custom_weights.keys():
                if category not in default_categories_ref:
                    print(f"Warning: Custom weight provided for non-applicable category: '{category}'. It will be ignored.")
        ...
        if is_all_years:
            overall_scores_by_year = {}
            interpretations_by_year = {} 
            all_years = set()
            
            # Populate all_years from the category_scores_data
            for category, data in category_scores_data.items():
                avg_scores_for_category = data.get("Average Category Score")
                if isinstance(avg_scores_for_category, dict):
                    all_years.update(avg_scores_for_category.keys())
            
            # Convert years to integers for sorting, then back to strings
            sorted_years = sorted([int(y) for y in all_years if str(y).isdigit()])
            sorted_years_str = [str(y) for y in sorted_years]

            for y in sorted_years_str: # Iterate through sorted years
                current_year_total_score = 0
                valid_weights_sum = 0
                ...
                for category, weight in weights.items():
                    ...
                    if isinstance(avg_scores_for_category, dict): # Check if it's the expected nested dict for 'all' years
                        category_avg_score = avg_scores_for_category.get(y)
                        
                        if category_avg_score is not None and pd.notna(category_avg_score): # Added pd.notna check
                            weighted_contribution = category_avg_score * weight
                            current_year_total_score += weighted_contribution
                            valid_weights_sum += weight
                ...
                if valid_weights_sum > 0:
                    ...
                    interpretations_by_year[y] = self._get_score_interpretation(None)
                
                # Store the detailed breakdown for this specific year within the overall breakdown structure
                # This needs to be structured carefully for 'all years' case.
                for cat_name, cat_data in current_year_breakdown_for_this_year.items():
                    if cat_name not in detailed_breakdown:
                        detailed_breakdown[cat_name] = {"Breakdown By Year": {}}
                    detailed_breakdown[cat_name]["Breakdown By Year"][y] = cat_data
            ...
            overall_score_result = overall_scores_by_year
            overall_score_interpretation = interpretations_by_year 

        else: # Specific year handling - Corrected this block
            final_overall_score = 0
            valid_weights_sum = 0 

            for category, weight in weights.items():
                # For a specific year, `category_scores_data.get(category)` should return
                # a dictionary like {"Average Category Score": VALUE, ...}
                category_data_for_year = category_scores_data.get(category, {})
                
                # Directly get the "Average Category Score" from this dictionary
                avg_score_for_year = category_data_for_year.get("Average Category Score")
                
                weighted_contribution = None
                if avg_score_for_year is not None and pd.notna(avg_score_for_year): # Check for NaN from pandas
                    weighted_contribution = avg_score_for_year * weight
                    final_overall_score += weighted_contribution
                    valid_weights_sum += weight
                
                detailed_breakdown[category] = {
                    "Average Category Score": avg_score_for_year,
                    "Weight Applied": weight,
                    "Weighted Contribution": weighted_contribution
                }
