...
class FinancialStatementAnalysisView(APIView):
    def get(self, request):
        try:
            ...
            # Case 1: When sector == 'All' and indicator != 'All' (existing code)
            if sector == 'All' and indicator != 'All':
                indicator_filter = df['Indicator'].str.contains(f"{re.escape(indicator)}", case=False, na=False)
            ...
            # Case 2: When org_name == 'All' and we need to calculate averages (new code)
            if sector != 'All' and org_name == 'All' and indicator != 'All':
                try:
                    ...
                    # Calculate averages by subsector
                    if sub_sector != 'All':
                        ...
                        # Calculate the subsector average
                        ...
                        # Handle all subsectors in the selected sector
                        subsector_avgs = filtered_df.groupby('Sub-Sector')[selected_years].apply(
                            lambda x: x.apply(pd.to_numeric, errors='coerce').mean()
                        ).reset_index()
                        ...
                        result = {
                            ...
                            'Combined Sector Average': {
                                year: round(float(sector_avg[year]), 2) if pd.notna(sector_avg[year]) else None
                                for year in selected_years
                            }
                        }
                        
                        for _, subsector_row in subsector_avgs.iterrows():
                            subsector_name = subsector_row['Sub-Sector']
                            subsector_data = {
                                'Sub-Sector': subsector_name,
                                'Organizations': []
                            }
                            ...
                            # Add organization data for this subsector
                            subsector_orgs = filtered_df[filtered_df['Sub-Sector'] == subsector_name]
                            for _, org in subsector_orgs.iterrows():
                                org_entry = {
                                    'Org_Name': org['Org Name']
                                }
                                for year in selected_years:
                                    value = org[year]
                                    org_entry[year] = round(float(value), 2) if pd.notna(value) else None
                                subsector_data['Organizations'].append(org_entry)
                            
                            result['Subsector_Averages'].append(subsector_data)
