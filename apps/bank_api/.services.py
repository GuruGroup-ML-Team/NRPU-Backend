...
class BaseDataService:
    """Base service for handling financial data operations"""
    ...
    def get_csv_path(filename):
        ...
        # Define possible locations for the CSV file
        possible_paths = [
            os.path.join(base_dir, filename),  # Project root
    ...
    @staticmethod
    def get_data(filename, org_key="Companies"):
        """
        Load and process data from CSV

        Args:
            filename (str): The name of the CSV file
            org_key (str): The key for the organization in the data dictionary

        Returns:
            dict: Processed data in nested dictionary format
        """
        try:
            # Get the path to the CSV file
            csv_file_path = BaseDataService.get_csv_path(filename)
            print(f"Attempting to load CSV from: {csv_file_path}")
            ...
            for _, row in df.iterrows():
                org_name = row['Org Name']
                item_name = row['Item Name'] if 'Item Name' in row else row['Indicator']

                # Create the organization if it doesn't exist yet
                if org_name not in data[org_key]:
                    data[org_key][org_name] = {}

                # Add the item data
                if item_name not in data[org_key][org_name]:
                    data[org_key][org_name][item_name] = {}

                # Add the yearly values
                for year in YEARS:
                    if year in df.columns and not pd.isna(row.get(year)):
                        data[org_key][org_name][item_name][year] = row[year]
    ...
    @staticmethod
    def get_listed_organizations(filename, org_key="Companies"):
        """
        Returns a list of organizations listed in the dataset
        ...
        try:
            ...
            organizations = df['Org Name'].unique().tolist()
            return organizations

        except Exception as e:
            print(f"Error getting listed organizations: {e}")
            return []
    ...
    @staticmethod
    def get_sub_sectors(filename, sector):
        """
        Returns a list of sub-sectors for a given sector
    ...
    @staticmethod
    def get_organizations_by_sub_sector(filename, sub_sector):
        """
        Returns a list of organizations for a given sub-sector
        ...
        try:
            ...
            # Filter by sub-sector and get unique organization names
            organizations = df[df['Sub-Sector'] == sub_sector]['Org Name'].unique().tolist()
            return organizations

        except Exception as e:
            print(f"Error getting organizations by sub-sector: {e}")
            return []
    ...
    @staticmethod
    def calculate_financial_ratios(data, year="2022"):
        """
        Calculate key financial ratios for an organization based on its data

        Args:
            data (dict): The organization's financial data
            year (str): The year to calculate ratios for
        ...
        try:
            # Helper function to safely calculate a ratio
            def calculate_ratio(numerator_key, denominator_key, factor=100):
                if (numerator_key in data and year in data[numerator_key] and
                        denominator_key in data and year in data[denominator_key] and
                        data[denominator_key][year] > 0):
                    numerator = data[numerator_key][year]
                    denominator = data[denominator_key][year]
                    return round((numerator / denominator) * factor, 2)
                return None
    ...
    @staticmethod
    def calculate_ratio_trend(data, ratio_type, numerator_key, denominator_key, factor=100, years=None):
        """
        Calculate trend data for a specific ratio over multiple years
        ...
        Args:
            data (dict): The organization's financial data
            ratio_type (str): Type of ratio (e.g., 'ROA', 'Net Profit Margin')
            numerator_key (str): Key for the numerator in data
            denominator_key (str): Key for the denominator in data
            factor (float): Multiplication factor (e.g., 100 for percentage)
            years (list): List of years to include in trend (defaults to all available)
        ...
        """
        if years is None:
            years = BaseDataService.get_years()
    ...
    @staticmethod
    def calculate_score(data, weights, year="2022"):
        """
        Calculate an overall financial health score for an organization
        ...
        """
        # Calculate all ratios first
        ratios = BaseDataService.calculate_financial_ratios(data, year)
        ...
        # For each category and ratio, assign score
        for category, category_ratios in ratios.items():
            for ratio_name, ratio_value in category_ratios.items():
                # Skip if ratio is None
                if ratio_value is None:
                ...
                # Assign score based on ratio value
                if category == PROFITABILITY_RATIOS:
                    if ratio_name == "return_on_equity":
                        if ratio_value >= 15:
                            score = 5
        ...
        return {
            ...
            "rating": rating,
            "comparison_data": {
                "organization_ratios": ratios,
                "industry_ratios": None  # Placeholder for industry data
            }
        }

    @staticmethod
    def get_comparison_data(filename, organizations, metrics, year="2022", org_key="Companies"):
        """
        Get comparison data for multiple organizations on specified metrics

        Args:
            filename (str): The name of the CSV file
            organizations (list): List of organization names to compare
            metrics (list): List of metrics to compare
            year (str): Year for comparison
            org_key (str): The key for the organization in the data dictionary

        Returns:
            dict: Comparison data organized by organization and metric
        """
        data = BaseDataService.get_data(filename, org_key)
        result = {}

        for org in organizations:
            if org not in data[org_key]:
                continue

            org_data = data[org_key][org]
            result[org] = {}

            for metric in metrics:
                if metric in org_data and year in org_data[metric]:
                    result[org][metric] = org_data[metric][year]
                else:
                    result[org][metric] = None

        return result


class CompanyDataService(BaseDataService):
    """Service for handling company financial data operations"""

    @staticmethod
    def get_company_data():
        """Load and process company data from CSV"""
        return BaseDataService.get_data(CSV_FILENAME, org_key="Companies")


class BankDataService(BaseDataService):
    """Service for handling bank financial data operations"""

    @staticmethod
    def get_bank_data():
        """Load and process bank data from CSV"""
        return BaseDataService.get_data(CSV_FILENAME, org_key="Banks")