...
#             self.df_pivot = pd.read_excel("Data/input_data.xlsx")
#             self.df_sector_avg = pd.read_excel("Data/altman_sector_averages.xlsx")
#             self.df_predictions = pd.read_excel("Data/predictions.xlsx") 
#             print("Datasets loaded successfully.")
#         except Exception as e:
#             print(f"Error loading datasets: {str(e)}")
#             self.df_pivot = pd.DataFrame()
...
#     def calculate_altman_score(self, df_filtered, data_source="input_data"):
        
#         print(f"Inside calculate_altman_score method (source: {data_source})")

#         if data_source == "input_data":
#             zscore_components = {
#                 '1. Capital work in progress': ('Sub Indicator', 'wc'),
#                 'Total Assets (A+B) / Equity & Liabilities (C+D+E)': ('Sub Indicator', 'ta'),
#                 'of which: un-appropriated profit(loss) / retained earnings': ('Sub-Sub Indicator', 're'),
...
#                         if isinstance(altman_zscore, np.floating) and (np.isnan(altman_zscore) or np.isinf(altman_zscore)):
#                             altman_scores[year_str] = None
#                         else:
#                             altman_scores[year_str] = float(altman_zscore)
                            
#                         print(f"Calculated Altman Z-Score for {year_str}: {altman_zscore} (Predicted)")
#                     except Exception as e:
#                         year_str = str(int(year))
#                         print(f"Error in calculating Altman Z-Score for year {year_str}: {str(e)}")
#                         altman_scores[year_str] = f"Error: {str(e)}"
...
#                 print(f"Year {year} detected as future year, using predictions data")
#                 return self.get_predictions_data(request)
...
#                     sub_sectors = []
#                     for sub_info in sector_info["sub_sectors"].items():
#                         sub_data = {
#                             "sub_sector": sub_info["sub_sector"],
#                             "years": sub_info["years"],
#                             "total_organizations": sub_info["total_organizations"]
...
#     def get_predictions_data(self, request):
#         """
#         Process and return Altman Z-Scores based on the predictions.xlsx data format
#         """
#         try:
#             sector = request.query_params.get('sector', None)
#             sub_sector = request.query_params.get('sub_sector', None)
...
#             print(f"Processing predictions data for: sector={sector}, sub_sector={sub_sector}, org_name={org_name}, year={year}")
            
#             filtered_df = self.df_predictions.copy()
...
#                 except ValueError:
#                     print(f"Warning: Could not convert year {year} to integer")
#                     filtered_df = filtered_df[filtered_df['Year'].astype(str) == year]
            
#             if filtered_df.empty:
#                 if org_name:
#                     print(f"Trying flexible match for org_name in predictions: {org_name}")
#                     matched_df = self.df_predictions[self.df_predictions['Org Name'].str.contains(org_name, case=False, na=False)]
#                     if not matched_df.empty:
#                         filtered_df = matched_df
#                         print(f"Found organization using flexible match in predictions: {matched_df['Org Name'].iloc[0]}")
            
#             if filtered_df.empty:
#                 return Response({"message": "No prediction data found matching the specified criteria."}, 
#                                 status=status.HTTP_404_NOT_FOUND)
            
#             # Verify that all necessary columns exist
#             required_columns = ['ebit', 'mve', 'ta', 'tl_d', 'tl_e', 'wc', 're']
#             missing_columns = [col for col in required_columns if col not in filtered_df.columns]
#             if missing_columns:
#                 return Response({"message": f"Missing required columns in predictions data: {missing_columns}"}, 
#                                 status=status.HTTP_400_BAD_REQUEST)
            
#             # Calculate Altman Z-Score
#             altman_scores = self.calculate_altman_score(filtered_df, data_source="predictions")
...
#                     "org_name": org_name,
#                     "year": year,
#                     "altman_zscore": altman_scores.get(year, "Insufficient data or error in calculation"),
#                     "is_predicted": True
#                 }
#             else:
#                 response_data = {
...
#         except Exception as e:
#             print(f"Error in get_predictions_data: {str(e)}")
#             return Response({"error": str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
